...
real eta = 0.07; // Initial learning rate
...
void *glove_thread(void *vid) {
    ...
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        if (feof(fin)) break;
        if (cr.word1 < 1 || cr.word2 < 1) { fprintf(stderr,"%d %d\n",cr.word1,cr.word2);continue; }
        real_data+=1;
...
int train_glove() {
    ...
    for (b = 0; b < num_iter; b++) {
        ...
        fprintf(stderr,"real data number is %lld",real_data);
        ...
        fprintf(stderr,"small %f\n",small);
    }
        int time_end=time(NULL);
        printf("calculate use %f mins\n",(double)(time_end-time_start)/60);
    free(pt);
    free(lines_per_thread);
