...
char *vocab_file, *file_head,*user_vocab;
...
int merge_write(CRECID new, CRECID *old, FILE *fout) {
   ...
   __uint128_t ran=llrand();
    Sec_CREC t;
    t.word1=old->word1;
    t.word2=old->word2;
    t.pt1=(__uint128_t)(long long) (old->val*T)-ran;
    t.pt2=ran;
    
    fwrite(&t, sizeof(Sec_CREC), 1, fout);
    *old = new;
    return 1; 
}
...
int get_cooccurrence() {
    ...
    long long a, j = 0, k, id, counter = 0, ind = 0, vocab_size, w1, w2, *lookup = NULL, *history = NULL,jd=0;
    char format[20], filename[200], str[MAX_STRING_LENGTH + 1];
    FILE *fid, *foverflow;
    FILE *zfid;
    real *bigram_table = NULL, r;

    HASHREC *htmp,*userhtmp, **vocab_hash = inithashtable();
    HASHREC ** user_hash=inithashtable();
    ...
    j=0;
    zfid=fopen(user_vocab,"r");
    while(fscanf(zfid,format,str,&id)!= EOF) {hashinsert(user_hash,str,++jd);}
    fclose(zfid);
    fprintf(stderr, "Reading vocab from file \"%s\"...", vocab_file);
    ...
    while (1) {
        ...
        userhtmp=hashsearch(user_hash,str);
        ...
        for (k = j - 1; k >= ( (j > window_size) ? j - window_size : 0 ); k--) { 
            ...
            if ( w1 < max_product/w2 ) { 
                ...
                if (symmetric > 0) bigram_table[lookup[w2-1] + w1 - 2] += distance_weighting ? 1.0/((real)(j-k)) : 1.0; 
            }
            else { 
                cr[ind].word1 = w1;
                cr[ind].word2 = w2;
    ...
    fclose(foverflow);
    free_resources(vocab_hash, cr, lookup, history, bigram_table);
    return merge_files(fidcounter + 1); 
}
...
int main(int argc, char **argv) {
    ...
    user_vocab=malloc(sizeof(char) * MAX_STRING_LENGTH);
    ...
    if ((i = find_arg((char *)"-uservocab", argc, argv)) > 0) strcpy(user_vocab, argv[i + 1]);
    else strcpy(file_head, (char *)"user2_vocab.txt");
